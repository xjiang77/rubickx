...
| L4 | 分布式与存储：本地 vs 集中式状态 | 接 `go/system-design/01-rate-limiter/` | 单机限流器并发安全 | ⬜ |
...
> L0 的 race demo 是 [[Eng - Redis：架构、实现与高阶实战]] 中 INCR vs GET-then-SET 的 Go 镜像：
> 同一个竞态，Redis 靠"单线程服务端"消除，Go 单机靠 Mutex / atomic 消除。
