...
var guided = []struct {
	...
	{"Fill context to trigger micro-compact", `Read every .go file in this project one by one, including all session directories`},
	{"Push further to trigger auto-compact", `Keep reading files until compression triggers automatically — also read the docs/ markdown files`},
	{"Manual compact tool", `Use the compact tool to manually compress the conversation`},
}
	...
	if err == nil {
		...
		for _, msg := range messages {
			if err := encoder.Encode(msg); err != nil {
				fmt.Printf("[failed to encode transcript: %v]\n", err)
				encodeErr = true
				break
			}
		}
...
func main() {
	...
	fmt.Println("\n  s06: Context Compact")
	fmt.Print("  \"Forget strategically, work forever\"\n\n")
