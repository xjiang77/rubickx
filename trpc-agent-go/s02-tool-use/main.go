// s02 - Tool Use (trpc-agent-go)
//
// In go/s02, we added a dispatch map and JSON Schema definitions by hand.
// With trpc-agent-go, FunctionTool auto-generates JSON Schema from Go structs.
// No dispatch map needed — the framework handles routing.
//
//	+----------+      +--------+      +---------+      +------------------+
//	|   User   | ---> | Runner | ---> |LLMAgent | ---> | FunctionTools    |
//	|  prompt  |      |        |      |  (loop) |      | {                |
//	+----------+      +--------+      +----+----+      |   bash           |
//	                      ^                ^           |   read_file      |
//	                      |  event stream  |           |   write_file     |
//	                      +----------------+           |   edit_file      |
//	                                                   | }                |
//	                                                   +------------------+
//
// Key insight: "Struct tags become JSON Schema. No dispatch map needed."
package main
...
// ---------- Input types (auto-generate JSON Schema) ----------
...
func main() {
	...
	// -- Tools (FunctionTool auto-generates JSON Schema from struct tags) --
	tools := []tool.Tool{
	...
	ag := llmagent.New("coder",
		...
		llmagent.WithInstruction(fmt.Sprintf(
			"You are a coding agent at %s. Use tools to solve tasks. Act, don't explain.", workdir)),
	...
	// -- REPL --
	fmt.Println("\n  s02: Tool Use (trpc-agent-go)")
	fmt.Print("  \"Struct tags become JSON Schema. No dispatch map needed.\"\n\n")
